...
class CourseCompletionService {
 public function refresh(int $courseId,int $userId): void {
  $course=Course::with(['lessons'=>fn($q)=>$q->where('is_active',true)->where('is_required',true)])->find($courseId);if(!$course)return;
  ...
  if($course->certificate_enabled){Certificate::firstOrCreate(['course_id'=>$courseId,'user_id'=>$userId],['number'=>'PED-'.now()->format('Ymd').'-'.strtoupper(Str::random(8)),'score'=>$avg,'issued_at'=>now()]);}
 }
}
