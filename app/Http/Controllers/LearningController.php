...
class LearningController extends Controller {
 ...
 public function complete(Request $r,Lesson $lesson,CourseCompletionService $completion){$enrollment=Enrollment::where('course_id',$lesson->course_id)->where('user_id',$r->user()->id)->first();abort_unless($enrollment||$r->user()->isAdmin(),403);$p=LessonProgress::firstOrCreate(['lesson_id'=>$lesson->id,'user_id'=>$r->user()->id]);$p->update(['status'=>'completed','started_at'=>$p->started_at?:now(),'completed_at'=>now()]);$completion->refresh($lesson->course_id,$r->user()->id);return back()->with('success','Урок отмечен как пройденный');}
}
