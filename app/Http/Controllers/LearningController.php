...
class LearningController extends Controller
{
    ...
    {
        $enrollments=Enrollment::with(['course.lessons'])->where('user_id',$request->user()->id)->latest('enrolled_at')->get();
    ...
    public function lesson(Request $request, Lesson $lesson)
    {
        ...
        $enrollment=Enrollment::where('course_id',$lesson->course_id)->where('user_id',$request->user()->id)->first();
        abort_unless($enrollment||$request->user()->isAdmin(),403);
        $lesson->load(['course','material','scormPackage','files']);
