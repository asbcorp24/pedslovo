...
<div class="container py-5">
    <section class="hero p-4 p-md-5 mb-5">
        <div class="row align-items-center">
            <div class="col-lg-8 position-relative" style="z-index:1">
                <div class="text-uppercase small fw-bold gold mb-2">{{ $home['home_badge'] ?? __('Учебная часть') }}</div>
                <h1 class="display-4 fw-bold">{{ $home['home_title'] ?? __('Педслово — цифровая образовательная среда училища') }}</h1>
                <p class="lead text-white-50">{{ $home['home_subtitle'] ?? __('Учебные материалы, курсы и цифровые сервисы для студентов и преподавателей.') }}</p>
                ...
                <a class="text-decoration-none text-dark" href="{{ route('section.show', $section) }}">
                    <div class="card section-card shadow-sm h-100">
                        <div class="card-body p-4">
                            ...
                            <h2 class="h4">{{ $section->localizedTitle() }}</h2>
                            <p class="text-muted">{{ $section->localizedDescription() ?: __('Программы, материалы и учебные направления') }}</p>
                ...
                <div class="col-md-6 col-lg-4">
                    <a href="{{ route('section.show', $specialty) }}" class="text-decoration-none text-dark">
                        <div class="card content-card shadow-sm h-100">
                            <div class="card-body p-4">
                                <h3 class="h5">{{ $specialty->localizedTitle() }}</h3>
                                <div class="d-flex flex-wrap gap-2 mt-3">
                                    @for($year = 1; $year <= 4; $year++)
                                        <span class="year-pill">{{ $year }} {{ __('курс') }}</span>
                                    @endfor
                                </div>
                                <div class="small text-muted mt-3">{{ $specialty->courses_count }} {{ __('учебных дисциплин') }}</div>
                            </div>
                        </div>
                    </a>
                </div>
    ...
    <section class="bg-white rounded-4 shadow-sm p-4 p-md-5 mb-5">
        <div class="row align-items-center">
            ...
            <div class="col-lg-7">
                <p class="lead text-muted mb-0">{{ $home['home_about_text'] ?? __('Педслово объединяет учебные материалы, курсы, результаты и цифровые сервисы учебной части.') }}</p>
            </div>
        </div>
    </section>
            ...
            @foreach($featuredCourses as $course)
                <div class="col-md-6 col-lg-3">
                    <a href="{{ route('courses.show', $course) }}" class="text-decoration-none text-dark">
                        <div class="card content-card shadow-sm h-100">
                            <div class="card-body">
                                <span class="badge text-bg-light">{{ $course->study_year ? $course->study_year . ' ' . __('курс') : __('курс') }}</span>
                                <h3 class="h6 mt-2">{{ __($course->title) }}</h3>
                                <div class="small text-muted">{{ $course->section ? $course->section->localizedTitle() : '' }}</div>
                            </div>
                        </div>
                    </a>
                </div>
                ...
                <div class="col-md-6">
                    <div class="card content-card shadow-sm">
                        <div class="card-body">
                            ...
                            <h3 class="h5 mt-2"><a href="{{ route('material.show', $material) }}" class="text-dark text-decoration-none">{{ __($material->title) }}</a></h3>
