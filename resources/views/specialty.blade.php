...
@section('title',$section->localizedTitle().' — 1–4 '.__('курс').' | Педслово')
@section('meta_description',__('Учебные материалы и дисциплины специальности').' '.$section->localizedTitle().' '.__('по 1, 2, 3 и 4 курсам.'))
...
<div class="container py-5">
    ...
    <div class="card card-soft mb-5"><div class="card-body p-4 p-lg-5"><div class="eyebrow">{{ __('Специальность') }}</div><h1 class="display-6 fw-bold text-wine">{{ $section->localizedTitle() }}</h1><p class="lead text-muted mb-0">{{ $section->localizedDescription() ?: __('Материалы учебной части распределены по годам обучения. Выберите курс, чтобы перейти к дисциплинам и учебным модулям.') }}</p></div></div>
        ...
        @for($y=1;$y<=4;$y++)
            <div class="tab-pane fade {{ $y===1 ? 'show active' : '' }}" id="year{{ $y }}"><div class="row g-3">
                ...
                @forelse($yearCourses as $course)
                    <div class="col-md-6 col-lg-4"><div class="card card-soft lift h-100"><div class="card-body p-4"><div class="small text-muted mb-2">{{ $y }} {{ __('курс') }}</div><h3 class="h5">{{ __($course->title) }}</h3><p class="text-muted small">{{ \Illuminate\Support\Str::limit($course->description ? __($course->description) : '',130) }}</p>
    ...
    @if($section->materials->count())
        <div class="mt-5"><h2 class="h3">{{ __('Общие материалы специальности') }}</h2><div class="list-group">
            @foreach($section->materials as $m)
                <a class="list-group-item list-group-item-action" href="{{ route('material.show',$m) }}">{{ __($m->title) }}</a>
            @endforeach
        </div></div>
    @endif
</div>
