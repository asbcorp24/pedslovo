...
<div class="container py-5">
    ...
    <div class="row mt-3">
        <div class="col-lg-9">
            ...
            @if($lesson->material)
                ...
                <div class="card border-0 shadow-sm mb-3"><div class="card-body p-4">
                    <div class="d-flex justify-content-between gap-2 align-items-start">
                        <div><div class="small text-muted mb-1">Материал портала</div><h4>{{ $lesson->material->title }}</h4></div>
                ...
                @foreach($lesson->links as $link)
                    ...
                    <div class="card border-0 shadow-sm mb-3"><div class="card-body p-3">
                        <div class="d-flex justify-content-between align-items-center mb-2 gap-2">
                            <div><strong>{{ $link->title ?: 'Видео' }}</strong> <span class="badge text-bg-light">{{ $link->provider }}</span> <span class="badge {{ $link->is_required ? 'text-bg-warning' : 'text-bg-light' }}">{{ $link->is_required ? __('ui.required') : __('ui.optional') }}</span></div>
                            <a class="btn btn-sm btn-outline-secondary" href="{{ $link->url }}" target="_blank" rel="noopener">Открыть отдельно</a>
                        </div>
                        @if($link->embed_url)
                            <div class="ratio ratio-16x9"><iframe src="{{ $link->embed_url }}" title="{{ $link->title ?: 'Видео урока' }}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div>
                        @else
                            <div class="alert alert-light border">Видео доступно по внешней ссылке: <a href="{{ $link->url }}" target="_blank" rel="noopener">{{ $link->url }}</a></div>
                        @endif
                        <div class="mt-3">
                ...
                <div class="row g-3 mb-4">
                    @foreach($lesson->scormPackages as $package)
                        ...
                        <div class="col-md-6"><div class="card border-success-subtle shadow-sm h-100"><div class="card-body p-4">
                            ...
                            @if($package->pass_score !== null)<div class="small text-muted mb-2">Проходной балл: {{ $package->pass_score }}</div>@endif
                            @if($package->max_attempts)<div class="small text-muted mb-3">Попыток: {{ $package->max_attempts }}</div>@endif
            ...
            @if($lesson->files->count())<h2 class="h4 mt-4">Материалы урока</h2>@endif
            @foreach($lesson->files as $file)
                ...
                <div class="card border-0 shadow-sm mb-3"><div class="card-body p-3">
                    ...
                    @else<div class="alert alert-light border">Документ доступен для просмотра или скачивания: <a href="{{ $file->url }}" target="_blank">{{ $file->original_name }}</a></div>@endif
        ...
        <div class="col-lg-3">
            <div class="card border-0 shadow-sm position-sticky" style="top:1rem"><div class="card-body">
                ...
                @elseif(!in_array($lessonProgress->status,['completed','passed']))
                    <form method="post" action="{{ route('learning.lesson.complete',$lesson) }}">@csrf<button class="btn btn-primary w-100">{{ __('ui.lesson_completed') }}</button></form>
                @endif
            </div></div>
        </div>
    </div>
